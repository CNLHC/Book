...
def MockDoubanApiResponse_normal(*args, **kwargs):
    class fake_get_method():
        def __init__(self,text,status_code=200):
            self.text=text
            self.status_code=status_code

    #注意函数会改变传入基地址，所以此处需要检查字串
    if MockURL.Wrong_Json in args[0]:
        return fake_get_method("hello")
    elif MockURL.normal in args[0]:
        return fake_get_method(text=normalJsonData)
    elif MockURL.Exceed_limtation in args[0]:
        return fake_get_method(text=exceedData,status_code=403)

class TestDoubanBook(unittest.TestCase):

    @mock.patch('api.douban.requests.get',side_effect=MockDoubanApiResponse_normal)
    def testDoubanBook_normal(self,mock_get):
        a = BookInfoByIsbn("sadkj",apiBaseUrl=MockURL.normal)
        self.assertTrue(isinstance(a,dict))
        self.assertNotEquals(len(a),0)

    @mock.patch('api.douban.requests.get',side_effect=MockDoubanApiResponse_normal)
    def testDoubanBook_wrongJson(self,mock_get):
        a = BookInfoByIsbn("sadkj",apiBaseUrl=MockURL.Wrong_Json)
        self.assertEquals(len(a),0)

    @mock.patch('api.douban.requests.get',side_effect=MockDoubanApiResponse_normal)
    def testDoubanBook_exceedLimations(self,mock_get):
        a = BookInfoByIsbn("sadkj",apiBaseUrl=MockURL.Exceed_limtation)
        self.assertEquals(len(a),0)
